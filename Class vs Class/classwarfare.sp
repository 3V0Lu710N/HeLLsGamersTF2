...
{
    name        = "Class Warfare (Class vs Class)",
    author      = "Tsunami (Edited by 3V0Lu710N)",
...
{
    CreateConVar("sm_classwarfare_version", PLUGIN_VERSION, "Class Warfare in TF2.", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	...
	RegConsoleCmd("sm_eventinfo", EventInfo, "");
...
public Event_PlayerClass(Handle event, const String:name[], bool dontBroadcast)
{
    ...
    if(!IsValidClass(iClient, iClass))
    {
        ...
        CPrintToChat(iClient, "%s%s%s%s%s%s", "{fullred} [HG]{white} ", ClassNames[iClass],  " Is Not An Option This Round! It's {crimson}Red ", ClassNames[g_iRedClass], " {white}vs {dodgerblue}Blue ", ClassNames[g_iBlueClass] );
...
{
	AdvMOTD_ShowMOTDPanel(client, "Class vs Class", "http://hellsgamers.com/calendar/event/812-tf2-class-vs-class-leadership-vs-members/", MOTDPANEL_TYPE_URL, true, true, true, OnMOTDFailure);
...
PrintStatus() {
    ...
    CPrintToChatAll("%s%s%s%s", "{fullred}[HG] {white}This is Class Warfare: {crimson}Red ", ClassNames[g_iRedClass], " {white}vs {dodgerblue}Blue ", ClassNames[g_iBlueClass] );
	CPrintToChatAll("{fullred}[HG] {white}Event Host: {dodgerblue}HG | Misc2008 [S]");
	CPrintToChatAll("{fullred}[HG] {white}Prizes:");
	CPrintToChatAll("{fullred}[HG] {white}1st Place: {dodgerblue}HG Platinum Account");
	CPrintToChatAll("{fullred}[HG] {white}2nd Place: {gold}HG Gold Account");
	CPrintToChatAll("{fullred}[HG] {white}3rd Place: {limegreen}50,000 Jailbreak Rep");
	CPrintToChatAll("{fullred}#### HeLLsGamers ####");
}
...
{
    SetupClassRestrictions();
    PrintToChatAll("%s%s%s%s%s", "{fullred}[HG]", "{white}Mid Round Class Change: {fullred}Red{white} ", ClassNames[g_iRedClass], " vs {dodgerblue}Blue{white} ", ClassNames[g_iBlueClass] );
}
